...
const AnniversaryPage: React.FC = () => {
  ...
  useEffect(() => {
    const saved = storage.get<Anniversary[]>(STORAGE_KEYS.ANNIVERSARIES, [
      { id: 'default-1', title: '恋爱纪念日', date: '2023-01-01', reminderDays: 1 }
    ])
  ...
  const handleAdd = () => {
    ...
    saveAnniversaries([...anniversaries, next])
    setNewTitle('')
    setNewDate('')
    setShowAdd(false)
  }
    ...
    <div className="p-4 space-y-6 bg-[#F2F2F7] min-h-full pb-28 overflow-y-auto">
      ...
      <div className="space-y-4">
        {anniversaries.map(item => {
          ...
          const title = days > 0 ? `还剩 ${days}` : days === 0 ? '就是今天' : `已过 ${Math.abs(days)}`
          ...
          return (
            <div key={item.id} className="bg-white rounded-2xl p-4 shadow-sm flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-2xl font-black text-pink-500">{title}</p>
