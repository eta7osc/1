  ...
  const mockPhotos = useMemo(
    () =>
      Array.from({ length: 20 }).map((_, i) => ({
        ...
        category: albums[i % albums.length]
      })),
    []
  )

  const visiblePhotos = mockPhotos.filter(photo => {
    if (activeTab === 0) {
      return true
    }
    return photo.category === albums[activeTab]
  })
  ...
  return (
    <div className="bg-[#F2F2F7] min-h-full pb-28">
      ...
      <div className="fixed bottom-[100px] right-6">
        <button
          type="button"
          className="w-14 h-14 bg-pink-500 rounded-full shadow-2xl flex items-center justify-center text-white active:scale-90 transition-transform"
        >
          <ImageIcon size={28} />
        </button>
      </div>
